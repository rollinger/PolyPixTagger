...
class PixTagMainWindow(QtWidgets.QMainWindow):
    ...
    def _build_actions(self):
        ...
        # Quick Save
        act_quick_save = QtGui.QAction(
            self.style().standardIcon(QtWidgets.QStyle.SP_DialogSaveButton),
            "Save Project",
            self,
        )
        act_quick_save.triggered.connect(self.quick_save_project_json)
        # Save project JSON
        ...
        m_file.addAction(act_quick_save)
        ...
        toolbar.addAction(act_quick_save)
        ...
        self.act_entity_point.triggered.connect(lambda: self.set_tool("entity_point"))
        ...
        # Build toolbar from list of actions
        toolbar_layout = (self.act_pan, self.act_probe, "|", self.act_erase, self.act_brush, "|", self.act_entity_point)
        group = QtGui.QActionGroup(self)
        for a in toolbar_layout:
            if isinstance(a, str):
                if a == "|":
                    toolbar.addSeparator()
            else:
    ...
    def on_layer_selected(self, current: Optional[QtWidgets.QListWidgetItem], prev):
        ...
        self.refresh_category_list()
        self.refresh_entity_list()
        self.show_current_layer_overlay()
        self.rebuild_entities()
        self.update_selection_label()
    ...
    def show_current_layer_overlay(self):
        # hide all
        ...
        if not isinstance(getattr(layer, "overlay_rgba_img", None), QtGui.QImage):
            ...
            full = QtCore.QRect(0, 0, self.project.image_width, self.project.image_height)
            self.update_layer_overlay(layer, full)  # one-time cost

        # ensure item exists
        item = self.layer_overlay_items.get(layer.id)
        if item is None:
            item = self.scene.addPixmap(QtGui.QPixmap.fromImage(layer.overlay_rgba_img))
            item.setZValue(10)
            item.setOpacity(0.55)
            item.setPos(0, 0)
            self.layer_overlay_items[layer.id] = item
        else:
            item.setPixmap(QtGui.QPixmap.fromImage(layer.overlay_rgba_img))
    ...
    def save_project_json(self, path=None):
        if not path:
